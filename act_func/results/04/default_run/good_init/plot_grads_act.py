...
def weight_distributions_per_layer(iterations,
                                   ...
                                   savepath='',
                                   rand=False):
    fig, axes = plt.subplots(1, len(iterations), sharey=True, figsize=(20, 6))
    # fig.tight_layout()
    if rand:
...
def kl_divergence_per_iteration(params, savepath):
    pre_hist = None
    klds = []
    fig, ax = plt.subplots(figsize=(6, 4))
    for i, k in enumerate(params):
        hist = np.histogram(params[k]['ensemble'])[0]
        if i > 0:
            kld = entropy(hist, pre_hist)
            klds.append(kld)
            print(kld)
        pre_hist = hist
    ax.plot(klds,  '.-')
    ax.set_xticks(range(14), range(500,7500, 500))
    tkl = ax.xaxis.get_ticklabels()
    for label in tkl[::2]:
        label.set_visible(False)
    # ax.set_xticks()
    ax.set_ylabel('DKL')
...
if __name__ == '__main__':
    ...
    kl_divergence_per_iteration(activations, savepath='enkf_kl_divergence.pdf')
