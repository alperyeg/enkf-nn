...
class EnsembleKalmanFilter(KalmanFilter):
    ...
    def fit(self, data, ensemble, ensemble_size, moments1,
        ...
        self.observations = _encode_multi_targets(observations, self.gamma_s)
        self.data = data.copy()
...
def _update_step(ensemble, observations, g, gamma, Cpp, Cup):
    ...
    Returns new ensembles
    """
    return ensemble + (Cup @ np.linalg.lstsq(Cpp+gamma, (observations - g).T / len(observations))[0]).T
...
def _get_shapes(observations, model_output):
    ...
    dimensions = observations[0].shape[0]
...
def _multi_one_hot_vector(targets, index, shape):
    one_hot = np.zeros(shape)
    for target in targets:
        one_hot[target[index]] = 1.0
    return one_hot
...
def _encode_multi_targets(targets, shape):
    return np.array(
        [_multi_one_hot_vector(targets, i, shape) for i in
         range(targets[0].shape[0])])
