    ...
    def set_parameters(self, ensembles):
        ...
        with torch.no_grad():
            ...
            for ii in range(len(labels)):
                ...
                #                               'MSE')
                sc = score(l_modded.numpy(),
                           np.argmax((outputs, outputs2)[ii].numpy(), 1))
                print('Individual score {} for label {}'.format(sc, ii))
                train_acc_total += sc
            ...
            for ii in range(len(labels)):
                ...
                #                               'MSE')
                sc = score(l_modded.numpy(),
                           np.argmax((test_output, test_output2)[ii].numpy(),
                                     1))
                print('Individual score {} for label {}'.format(sc, ii))
                test_acc_total += sc
            test_acc_total /= len(labels)
