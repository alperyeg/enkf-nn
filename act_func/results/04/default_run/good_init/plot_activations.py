...
if __name__ == '__main__':
    ...
    for file in os.listdir(path):
        if file.endswith('6000.npy'):
            conv_params[file] = np.load(file, allow_pickle=True).item()
            files.append(file)
    ...
    # weight_distributions_per_layer(iters, savepath='dist_ensembles_iterations0_rand.pdf', rand=True)
    activation_functions_dist_iteration(conv_params['conv_params_6000.npy']['act_func'])
    activations_mean_std_error(conv_params['conv_params_6000.npy']['act_func'])
